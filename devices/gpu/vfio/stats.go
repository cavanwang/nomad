...
func (d *VfioGpuDevice) stats(ctx context.Context, stats chan<- *device.StatsResponse, interval time.Duration) {
	defer close(stats)
...
func (d *VfioGpuDevice) writeStatsToChannel(stats chan<- *device.StatsResponse, timestamp time.Time) {
	...
	// filter only stats from devices that are stored in VfioGpuDevice struct
	d.deviceLock.RLock()
